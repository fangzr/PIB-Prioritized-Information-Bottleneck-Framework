...
class sequenceDataset_delay(VisionDataset):
    ...
    def get_single_frame_data(self, frame):
        imgs = []
        for cam in range(self.num_cam):
            min_frame = self.min_frame  # Gets the minimum value of the frame_range         
        ...
        # delayed_frames_gt = [max(min_frame, frame - delay) for delay in self.delays]
        ...
        imgs_gt = []
        for cam in range(self.num_cam):
            # delayed_frame = max(min_frame, frame - self.delays[cam])
