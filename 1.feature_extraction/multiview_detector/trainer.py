...
class PerspectiveTrainer(BaseTrainer):
    ...
    def train(self, epoch, data_loader, optimizer, delays, log_interval=100, cyclic_scheduler=None):
        ...
        gt_losses = 0 
        bits_losses = 0
        weighted_bits_loss = 0

        target_rate = self.model.args.target_rate
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        target_rate = torch.FloatTensor([target_rate]).to(device)

        precision_s, recall_s = AverageMeter(), AverageMeter()
        
        # Generate camera_weights based on the delays entered and apply them to the calculation of the bits_loss
        ...
        for batch_idx, (data, map_gt, _) in enumerate(data_loader):
            ...
            # Calculate the gt_loss for 1-tau, starting from 1 because 0 has already been accounted for.
            ...
            # Calculate weighted bits_loss
            camera_weights = self.weights_generator(delays)
            # weighted_bits_loss = torch.sum(bits_loss * camera_weights)
            weighted_bits_loss = bits_loss * camera_weights
            
            
            loss = gt_weighted_loss + torch.max(weighted_bits_loss, target_rate) * self.beta 
            
            loss = self.calculate_loss(self.weights_generator,camera_weights,delays,loss,1e-3)
            ...
            if (batch_idx + 1) % log_interval == 0:
                ...
                print('weighted_bits_loss: {:.6f}, gt_weighted_loss: {:.6f}'.format(weighted_bits_loss.sum().item(), gt_weighted_loss.sum().item()))
                print(f"camera_weights: {camera_weights}") 
                # self.print_camera_weights()
                # for camera in camera_weights:
                #     print(f"Camera: {camera.item()}, Weights: {camera_weights[camera.item()]}")
